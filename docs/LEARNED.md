...
> 37 条面试可讲点按主题归组（收官整理 2026-08-30）。正文在各阶段的「面试可讲」小节，条目标题可全文检索定位。

- **认证与授权 · RBAC（13）**：默认安全的守卫设计 / 单 token 滑动续期 + 前端半场（前后端合讲）/ 守卫为什么不在 beforeLoad 里验 token 有效性 / 一份权限点数据的前后端双投影 / 四码门控矩阵 / 门控粒度是后端接口码分布的投影 / 门控矩阵收官（悬空码与接口码正交性）/ 权限授予界面为什么要 checkStrictly / 权限门控与 PATCH 语义的三层咬合 / 数据层门控补全三级门控的第四位（enabled 双闸）/ “登录成功即 403”死角分析 / 验证码闭环的三道防线 → [后端](#后端阶段2026-07-30-起) · [前端](#前端阶段)
- **数据一致性与状态设计（9）**：无外键表的删除一致性 / 订单三件套设计 / 删除策略三选一 / 落库快照 vs 实时推导的对账模式 / 派生状态字段的 UI 三原则 / 金额的“权威在服务端”要做成协议而不是约定 / 看板聚合接口的四个设计题 / 列表页的“无状态组件”论 / 种子数据的完整性也是“clone 即跑”的一部分 → [后端](#后端阶段2026-07-30-起) · [前端](#前端阶段) · [挂账清账](#挂账清账2026-08-30)
- **契约与校验链路（5）**：后端代码作为类型单一事实来源 / 错误归一化的分层 / 双层校验的分工与实证手法 / multipart 上传的 Content-Type 是“谁都不能手写”的头 / excel 导入的错误呈现是契约形状驱动的 → [契约链路](#契约链路阶段) · [前端](#前端阶段)
- **HTTP 语义与安全面（3）**：GET 副作用是真实事故源 / 请求日志的“摘要 vs 全量”取舍 / 上传安全的最小闭环 → [后端](#后端阶段2026-07-30-起)
- **工程化与 DX（4）**：多人协作工程化两道闸设计 / dev/prod 环境形态分治 / 三合一 check 的工具链换代 / 分包优化的诚实工程学 → [骨架期](#骨架期2026-07-30) · [工程化收尾](#工程化收尾阶段2026-08-29)
- **测试与验证方法论（3）**：测试边界是选出来的，不是越多越好 / CI 门禁的分层演进 / 验收数据自给自足的实证手法 → [后端自动化测试](#后端自动化测试2026-08-30) · [前端](#前端阶段)
