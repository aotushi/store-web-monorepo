...
### 5 工程化收尾 ✅（2026-08-29 ~ 08-30；websee / crawler 按 PLAN 预留砍掉）
...
| 2026-08-30 | 全局链路装配抽 `setup-app.ts`，main.ts 与 e2e 同用                                | ValidationPipe/壳/过滤器原是 bootstrap 手工挂载，e2e 自行复制迟早漂移；同源函数与 swagger.ts 是同一决策家族               |
| 2026-08-30 | 单测直构守卫/服务 + 手写依赖桩，不起 Nest 测试容器；e2e 才用 @nestjs/testing      | 守卫与 service 都是构造注入纯类（PLAN §6.4"service 不碰 req/res"红利）；两种测试形态各展示一种，薄的优先                  |
